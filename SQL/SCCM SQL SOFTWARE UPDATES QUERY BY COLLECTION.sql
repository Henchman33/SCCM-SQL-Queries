DECLARE @CollectionID NVARCHAR(8) = 'P010010E'
...
SELECT
    ...
    -- IPv4 from RA table (most reliable)
    STUFF((
...
SELECT 
    ...
    SUM(CASE WHEN ucs.Status = 3 THEN 1 ELSE 0 END) AS TotalInstalledUpdates,
    SUM(CASE WHEN ucs.Status = 2 THEN 1 ELSE 0 END) AS TotalMissingUpdates,

    CAST(
        (SUM(CASE WHEN ucs.Status = 3 THEN 1 ELSE 0 END) * 100.0)
        / NULLIF(COUNT(ucs.Status),0)
    AS DECIMAL(5,2)) AS CollectionCompliancePercent
